        ...
        if self.prompt_pool:
            ...
            if self.use_prefix_tune_for_e_prompt:
                ...
                if self.same_key_value:
                    prompt_pool_shape = (self.num_layers, 1, self.pool_size, self.length, embed_dim)
                    ...
                    prompt_pool_shape = (self.num_layers, 2, self.pool_size, self.length, embed_dim)
                    if prompt_init == 'zero':
                        self.prompt = nn.Parameter(torch.zeros(prompt_pool_shape))
                    elif prompt_init == 'uniform':
    ...
    def forward(self, x_embed, prompt_mask=None, cls_features=None):
        ...
        if self.prompt_pool:
            ...
            if self.use_prefix_tune_for_e_prompt:
                ...
                num_layers, dual, batch_size, top_k, length, embed_dim = batched_prompt_raw.shape
                batched_prompt = batched_prompt_raw.reshape(
                    num_layers, batch_size, dual, top_k * length, embed_dim
                )
            else:
            ...
            if self.use_prefix_tune_for_e_prompt:
                assert embed_dim % self.num_heads == 0
                if self.same_key_value:
                    prompt_pool_shape = (self.num_layers, 1, self.length, embed_dim)
