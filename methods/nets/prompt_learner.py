...
class CLAPTextEncoder(nn.Module):
    def __init__(self, cfg, classnames, tmp, is_weak):
        super().__init__()
        self.opt = cfg
    ...
    def forward(self):
        x = self.text_branch(
            input_ids=self.text["input_ids"].to(device),
            attention_mask=self.text["attention_mask"].to(device),
        )["pooler_output"]
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model, is_weak):
        super().__init__()
        n_cls = len(classnames)
