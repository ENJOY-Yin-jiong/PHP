...
#with torch.no_grad():
    def forward_features(self, x, longer_idx=None):
        ...
        # a new variable to store the output from blocks and the attention from blocks
        mid_output = []
        ...
        for i, layer in enumerate(self.layers):
            x, attn = layer(x)
            mid_output.append(x)
            mid_attention_output.append(attn)
        # for x
        x = self.norm(x)
        B, N, C = x.shape
        ...
        output_dict = {
            ...
            'swin_transformer_output': mid_output,
            'swin_transformer_attention_output': mid_attention_output
        }
...
class Attention(nn.Module):
    def __init__(self, keys_size=1024, queries_size=1024, num_hiddens=1024, dropout=0.1, **kwargs):
        super(Attention, self).__init__(**kwargs)
        # queries_size,keys_size都是输入的序列的长度，自注意力就是一样的长度，比如都是音频的长度,但是需要对齐视频的长度
        self.W_q = nn.Linear(queries_size, num_hiddens, bias=False)
        self.W_k = nn.Linear(keys_size, num_hiddens, bias=False)
        self.W_v = nn.Linear(num_hiddens, 1, bias=False)
        self.dropout = nn.Dropout(dropout)

    # 使用自注意力，因此queries和keys应该是同一个变量
...
# 音频对视频做attention的adaptor
class VideoAudioAttentionAdapter(nn.Module):
    def __int__(self, inputDim, outputDim, inputChannel, outputChannel):
        # Attention的参数暂时未知，根据给出的输入调整
        self.inputdim = inputDim
        self.outputdim = outputDim
        self.inputchannel = inputChannel
        self.outputchannel = outputChannel
        self.alignment = FitModel(inputDim, outputDim, inputChannel, outputChannel)
        self.attention = Attention()

    def forward(self, videooutput, audiooutput):
        return self.attention(audiooutput, audiooutput, videooutput)
...
class FitModel(nn.Module):
    def __int__(self, inputDim, outputDim, inputChannel, outputChannel):
        ...
        self.inputchannel = inputChannel
        self.outputchannel = outputChannel
        self.linear = nn.Linear(in_features=inputDim, out_features=inputDim)
        self.conv = nn.Conv2d(in_channels=inputChannel, out_channels=outputChannel, kernel_size=3, padding=True)
    def forward(self, x):
        x=self.linear(x)
        x=self.conv(x)
        return x
