...
def edge_aware_loss(pred, gt, epsilon=1e-8):
    pred_edge = F.conv2d(pred, torch.ones(1,1,3,3,device=pred.device)/9)
    gt_edge = F.conv2d(gt, torch.ones(1,1,3,3,device=gt.device)/9)
...
def IouSemanticAwareLoss(pred_masks, first_gt_mask, \
    ...
    # indices = torch.tensor(list(range(0, len(pred_masks), 5))).to(pred_masks.device)
    # first_pred_masks = torch.index_select(pred_masks, dim=0, index=indices)
    
    # # 调整真实标签维度
    # if len(first_gt_mask.shape) == 5:
    #     first_gt_mask = first_gt_mask.squeeze(1)  # [bs, 1, 224, 224]
    # edge_loss = edge_aware_loss(first_pred_masks, first_gt_mask.squeeze(1))
    # total_loss += 0.5 * edge_loss  # 调节权重
